    ...
    <header className=" w-screen flex flex-col">
      ...
      <div className="w-full max-w-[1400px] mx-auto px-4 flex  items-center justify-between">
        ...
        <nav className="hidden lg:flex gap-6">
          <Link className={`text-xl ${pathname === "/" ? 'font-bold' : ''} hover:underline`} href="/">
            Strona główna
          </Link>
          <Link className={`text-xl ${pathname === "/about" ? 'font-bold' : ''} hover:underline`} href="/about">
            O nas
          </Link>
          <Link className={`text-xl ${pathname === "/shop" ? 'font-bold' : ''} hover:underline`} href="/shop">
            Sklep
          </Link>
        </nav>
        ...
        <nav className="flex gap-4 ml-[20px]">
          <Link href="/">
            <img className="h-[32px] hidden lg:block" src="/user.svg" alt="User" />
          </Link>
          <Link href="/cart">
        ...
        <button className="lg:hidden h-[32px] pt-[1px]">
          <img className="h-full text-[var(--third-color)] fill-gray-500" src="/menu.svg" alt="Menu" />
        </button>
        </div>
      </div>
    </header>
